...
export default async function CharlottetownCityPage() {
  const profile = getTownProfile(SLUG);
  if (!profile) notFound();
  ...
  const [dailyForecast, nearbyData, mapStubs] = await Promise.all([
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:py-10 space-y-10">
        ...
        <section className="space-y-4">
          ...
          <TownIntelGrid
            ...
            location={data.location}
            weather={data.weather}
            charlottetownWeather={data.weather}
            waterTemp={waterTemp}
            tides={data.tide}
          />
        </section>
        ...
        {/* ── 14. SUBSCRIBE ────────────────────────────────────────────── */}
        <SubscribeStrip source="city-charlottetown" />
