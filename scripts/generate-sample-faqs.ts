...
function assertGatewayAuth(): void {
  ...
  if (!hasApiKey && !hasOidc) {
    console.error(
      "Missing AI Gateway credentials for local runs.\n" +
        "  • Set AI_GATEWAY_API_KEY (Vercel dashboard → AI → API keys), or\n" +
        "  • Set VERCEL_OIDC_TOKEN (e.g. run `vercel env pull` after enabling OIDC on the project).\n" +
        "On Vercel deployments, OIDC is provided automatically when AI Gateway is enabled.",
    );
    process.exit(1);
  }
}
...
Be specific, reference the actual numbers above, mention what activities are good/bad, and make it sound like a knowledgeable local. Do NOT include markdown or any text outside the JSON object.`;
    ...
    try {
      const { text } = await generateText({
        ...
        maxOutputTokens: 300,
        system: "You respond only with valid JSON objects, no markdown fences.",
        prompt,
      });

      const faq = parseJsonFromModel(text) as { question: string; answer: string };
      allFAQs.push({
